...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function navigateToPage(pageId) {
      // Ẩn tất cả các trang, bao gồm cả edit roadmap
      pages.forEach(page => page.classList.add('hidden'));
      
      // Xóa active class khỏi tất cả menu items
      menuItems.forEach(mi => mi.classList.remove('active'));
      
      // Set active class cho menu item tương ứng với trang
      ...
      // Đảm bảo edit roadmap page luôn ẩn khi chuyển trang
      if (pageId !== 'roadmaps' && editRoadmapPage) {
          editRoadmapPage.classList.add('hidden');
      }
  }
  
  // Set event listener cho menu items
  ...
  // Xác định trang mặc định khi load
  function setDefaultPage() {
      // Kiểm tra URL để xem có page được chỉ định
      ...
      if (pageParam) {
          ...
          // Nếu không có param, mặc định chọn trang đầu tiên
          const firstMenuItem = menuItems[0];
          if (firstMenuItem) {
              const firstPageId = firstMenuItem.getAttribute('data-page');
              navigateToPage(firstPageId);
          }
      }
      
      // Đảm bảo edit roadmap page luôn ẩn khi load trang
      if (editRoadmapPage) {
          editRoadmapPage.classList.add('hidden');
      }
  }
  
  // Thêm hàm này vào global scope để các module khác có thể sử dụng
